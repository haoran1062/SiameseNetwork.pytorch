...
class ClassifyDataset(data.Dataset):
    ...
    def get_cls_pathlist_map(self):
        ...
        for now_path in tqdm(self.fnames):
    ...
    def __getitem__(self,idx):
        ...
        # choice positive
        fname = self.cls_path_map[label1][randint(0, len(self.cls_path_map[label1])-1)]
        ...
        ch_label_list.remove(label1)
        label3 = ch_label_list[randint(0, len(ch_label_list)-1)]
        fname = self.cls_path_map[label3][randint(0, len(self.cls_path_map[label3])-1)]
        img3 = self.get_img_from_path(fname)
        assert img3 is not None, print(fname)

        if self.train:
            # add data augument
            seq_det = self.augmentation.to_deterministic()
            img3 = seq_det.augment_images([img3])[0]
